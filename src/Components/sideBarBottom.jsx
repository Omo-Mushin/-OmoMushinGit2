  ...
  return (
    <div className='side-bar-bottom'>
      <section className="container" style={{ cursor: 'pointer' }}>
        {props.persons.map(post => (
          <li className='list_group' key={post.id}>
            <article className="review">
              ...
              <NavLink to={`/post/${post.name}/${post.id}/${post.text}/${post.job}`}>
                <p className="info">
                  {post.text}
                </p>
              </NavLink>
              <div className='delete-like-group'>
                {/* <Like onClick={() => props.onLike(post)} liked={post.liked} /> */}
                <button type="button" className="btn btn-danger m-2" onClick={() => props.onDelete(post)}>Delete</button>
              </div>
            </article>
          </li>
        ))}
      ...
      <div className='pagn'>
        <StripePagination
          onSwitch={props.switchPage}
          itemCount={props.reviews.length}
