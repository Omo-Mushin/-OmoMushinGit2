...
import { InfoList, getInfoList } from '../info';
...
const MainApp = () => {
  ...
  const [onList, setOnList] = useState(getInfoList());
  ...
  const formatPostdata = (postsData) => {
    const postData = [];
    for (let key in postsData) {
      postData.push({ ...postsData[key], id: key });
    }
    setPosts(postData);
  };

  const handleSwitchList = (listItem) => {
    setList(listItem);
  };
  ...
  const handleDelete = async (post) => {
    try {
      await Axios.delete(`${apiEndPoint}/${post.name}/${post.id}.json`);
  ...
  const handleLogOut = async () => {
    ...
    // Handle redirection to the home page or another route after logout.
  };

  const persons = paginate(posts, currentPage, pageSize);
      ...
      <div className='app-body'>
        <div className='feed'>
          <Switch>
            ...
            <Route path='/postTest' component={Post} />
